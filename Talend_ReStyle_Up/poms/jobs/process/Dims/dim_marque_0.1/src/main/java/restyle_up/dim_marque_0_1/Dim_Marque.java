...
package restyle_up.dim_marque_0_1;
 ...
 * @status
 */
public class Dim_Marque implements TalendJob {
	...
	private final String jobName = "Dim_Marque";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Dim_Marque.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class out1Struct implements routines.system.IPersistableRow<out1Struct> {
		final static byte[] commonByteArrayLock_RESTYLE_UP_Dim_Marque = new byte[0];
		static byte[] commonByteArray_RESTYLE_UP_Dim_Marque = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_RESTYLE_UP_Dim_Marque.length) {
					if (length < 1024 && commonByteArray_RESTYLE_UP_Dim_Marque.length == 0) {
						commonByteArray_RESTYLE_UP_Dim_Marque = new byte[1024];
					} else {
						commonByteArray_RESTYLE_UP_Dim_Marque = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_RESTYLE_UP_Dim_Marque, 0, length);
				strReturn = new String(commonByteArray_RESTYLE_UP_Dim_Marque, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_RESTYLE_UP_Dim_Marque, 0, length);
				strReturn = new String(commonByteArray_RESTYLE_UP_Dim_Marque, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_RESTYLE_UP_Dim_Marque) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = ("Dim_Marque");
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "\".\"" + ("Dim_Marque");
				}
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:fwM+axWHaVUR5079e6zZaF9c1g1C/vDeyqpq9rnhgu1YdK+a");
				...
				try (java.sql.ResultSet rsTable_tDBOutput_1 = dbMetaData_tDBOutput_1.getTables(null, null, null,
					...
					while (rsTable_tDBOutput_1.next()) {
						...
						if (table_tDBOutput_1.equals(("Dim_Marque")) && (schema_tDBOutput_1.equals(dbschema_tDBOutput_1)
				...
				try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
					stmtCreate_tDBOutput_1.execute("CREATE TABLE \"" + tableName_tDBOutput_1
							+ "\"(\"ID_marque\" INT4 ,\"Marque\" VARCHAR(255)  )");
				}
				String insert_tDBOutput_1 = "INSERT INTO \"" + tableName_tDBOutput_1
						+ "\" (\"ID_marque\",\"Marque\") VALUES (?,?)";
...
// ###############################
				...
				final String decryptedPassword_tDBInput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:lG2PoIyEJbcpjo9FmC6vkZCK0quatenlPz+toPkBNP6VbeGn");
				...
				try {
					...
					while (rs_tDBInput_1.next()) {
						...
						row3 = null;
						if (row2.Marque == null) {
							finder_tUniqRow_1.Marque = null;
						} else {
							finder_tUniqRow_1.Marque = row2.Marque.toLowerCase();
						}
						...
						if (!keystUniqRow_1.contains(finder_tUniqRow_1)) {
							...
							if (row2.Marque == null) {
								new_tUniqRow_1.Marque = null;
							} else {
								new_tUniqRow_1.Marque = row2.Marque.toLowerCase();
							}
...
// # Output table : 'out1'
								out1_tmp.ID_marque = Numeric.sequence("s1", 1, 1);
								out1_tmp.Marque = row3.Marque;
								out1 = out1_tmp;
// ###############################
...
// Start of branch "out1"
							if (out1 != null) {
								...
								if (out1.ID_marque == null) {
									...
									pstmt_tDBOutput_1.setInt(1, out1.ID_marque);
								}

								if (out1.Marque == null) {
									pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(2, out1.Marque);
								}
...
// ###############################      
	...
	public static void main(String[] args) {
		final Dim_Marque Dim_MarqueClass = new Dim_Marque();

		int exitCode = Dim_MarqueClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Dim_Marque.class.getClassLoader()
					.getResourceAsStream("restyle_up/dim_marque_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Dim_Marque.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Dim_Marque");
		}
...
/************************************************************************************************
 * 80869 characters generated by Talend Open Studio for Data Integration on the
 * 12 avril 2025 à 8:38:36 PM CET
 ************************************************************************************************/