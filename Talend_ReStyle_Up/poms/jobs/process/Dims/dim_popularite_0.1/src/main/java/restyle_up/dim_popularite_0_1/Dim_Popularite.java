...
package restyle_up.dim_popularite_0_1;
 ...
 * @status
 */
public class Dim_Popularite implements TalendJob {
	...
	private final String jobName = "Dim_Popularite";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					Dim_Popularite.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Dim_Popularite.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class out1Struct implements routines.system.IPersistableRow<out1Struct> {
		final static byte[] commonByteArrayLock_RESTYLE_UP_Dim_Popularite = new byte[0];
		static byte[] commonByteArray_RESTYLE_UP_Dim_Popularite = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_RESTYLE_UP_Dim_Popularite.length) {
					if (length < 1024 && commonByteArray_RESTYLE_UP_Dim_Popularite.length == 0) {
						commonByteArray_RESTYLE_UP_Dim_Popularite = new byte[1024];
					} else {
						commonByteArray_RESTYLE_UP_Dim_Popularite = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_RESTYLE_UP_Dim_Popularite, 0, length);
				strReturn = new String(commonByteArray_RESTYLE_UP_Dim_Popularite, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_RESTYLE_UP_Dim_Popularite, 0, length);
				strReturn = new String(commonByteArray_RESTYLE_UP_Dim_Popularite, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_RESTYLE_UP_Dim_Popularite) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = ("Dim_Popularite");
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "\".\"" + ("Dim_Popularite");
				}
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:kndtzeWZZxy72PwIecsXkDYFXBYoij6HR9wA90zdY4PLCzxB");
				...
				try (java.sql.ResultSet rsTable_tDBOutput_1 = dbMetaData_tDBOutput_1.getTables(null, null, null,
					...
					while (rsTable_tDBOutput_1.next()) {
						...
						if (table_tDBOutput_1.equals(("Dim_Popularite"))
				...
				try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
					stmtCreate_tDBOutput_1.execute("CREATE TABLE \"" + tableName_tDBOutput_1
							+ "\"(\"ID_Popularite\" INT4 ,\"Popularite\" VARCHAR(255)  )");
				}
				String insert_tDBOutput_1 = "INSERT INTO \"" + tableName_tDBOutput_1
						+ "\" (\"ID_Popularite\",\"Popularite\") VALUES (?,?)";
...
// ###############################
				...
				final String decryptedPassword_tDBInput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:G2+Xp43z0KXntNgxWYYtP5ZQLmMG3bIinJfwtzYfttQ9KYii");
				...
				try {
					...
					while (rs_tDBInput_1.next()) {
						...
						row2.Popularite = row1.Popularite;
						...
						String searchStr_tReplace_1_1 = "Moyennene" + "";
						row2.Popularite = StringUtils.replaceAllStrictly(row2.Popularite, searchStr_tReplace_1_1,
								"moyenne" + "", true, false);
						String searchStr_tReplace_1_2 = " " + "";
						row2.Popularite = StringUtils.replaceAllStrictly(row2.Popularite, searchStr_tReplace_1_2,
								"" + "", true, false);
						row4.Popularite = row2.Popularite;
						...
						ope_tFilterRow_1.matches((row4.Popularite != null), "Popularite!=null failed");
						ope_tFilterRow_1.matches(
								(row4.Popularite == null ? false : row4.Popularite.compareTo(" ") != 0),
								"Popularite.compareTo(\" \") != 0 failed");

						if (ope_tFilterRow_1.getMatchFlag()) {
							if (row5 == null) {
								row5 = new row5Struct();
							}
							row5.Popularite = row4.Popularite;
...
// Start of branch "row5"
						if (row5 != null) {
							...
							row3 = null;
							if (row5.Popularite == null) {
								finder_tUniqRow_1.Popularite = null;
							} else {
								finder_tUniqRow_1.Popularite = row5.Popularite.toLowerCase();
							}
							...
							if (!keystUniqRow_1.contains(finder_tUniqRow_1)) {
								...
								if (row5.Popularite == null) {
									new_tUniqRow_1.Popularite = null;
								} else {
									new_tUniqRow_1.Popularite = row5.Popularite.toLowerCase();
								}
								...
								row3.Popularite = row5.Popularite;
...
// # Output table : 'out1'
									out1_tmp.ID_Popularite = Numeric.sequence("s1", 1, 1);
									out1_tmp.Popularite = row3.Popularite;
									out1 = out1_tmp;
// ###############################
...
// Start of branch "out1"
								if (out1 != null) {
									...
									if (out1.ID_Popularite == null) {
										...
										pstmt_tDBOutput_1.setInt(1, out1.ID_Popularite);
									}

									if (out1.Popularite == null) {
										pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
									} else {
										pstmt_tDBOutput_1.setString(2, out1.Popularite);
									}
...
// ###############################      
	...
	public static void main(String[] args) {
		final Dim_Popularite Dim_PopulariteClass = new Dim_Popularite();

		int exitCode = Dim_PopulariteClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Dim_Popularite.class.getClassLoader()
					.getResourceAsStream("restyle_up/dim_popularite_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Dim_Popularite.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : Dim_Popularite");
		}
...
/************************************************************************************************
 * 97931 characters generated by Talend Open Studio for Data Integration on the
 * 12 avril 2025 à 9:01:39 PM CET
 ************************************************************************************************/