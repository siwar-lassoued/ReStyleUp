...
package restyle_up.dim_collection_0_1;
 ...
 * @status
 */
public class Dim_Collection implements TalendJob {
	...
	private final String jobName = "Dim_Collection";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					Dim_Collection.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Dim_Collection.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class out1Struct implements routines.system.IPersistableRow<out1Struct> {
		final static byte[] commonByteArrayLock_RESTYLE_UP_Dim_Collection = new byte[0];
		static byte[] commonByteArray_RESTYLE_UP_Dim_Collection = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_RESTYLE_UP_Dim_Collection.length) {
					if (length < 1024 && commonByteArray_RESTYLE_UP_Dim_Collection.length == 0) {
						commonByteArray_RESTYLE_UP_Dim_Collection = new byte[1024];
					} else {
						commonByteArray_RESTYLE_UP_Dim_Collection = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_RESTYLE_UP_Dim_Collection, 0, length);
				strReturn = new String(commonByteArray_RESTYLE_UP_Dim_Collection, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_RESTYLE_UP_Dim_Collection, 0, length);
				strReturn = new String(commonByteArray_RESTYLE_UP_Dim_Collection, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_RESTYLE_UP_Dim_Collection) {
	...
	public static class roowStruct implements routines.system.IPersistableRow<roowStruct> {
		final static byte[] commonByteArrayLock_RESTYLE_UP_Dim_Collection = new byte[0];
		static byte[] commonByteArray_RESTYLE_UP_Dim_Collection = new byte[0];
		 ...
		 * Compare keys
		 */
		public int compareTo(roowStruct other) {
	...
	public void tDBInput_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				roowStruct roow = new roowStruct();
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = ("Dim_Collection");
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "\".\"" + ("Dim_Collection");
				}
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:qzzL6zcy1OZBz62TkqI48B6A5OcVWqp4p9suHQxshGqh5An/");
				...
				try (java.sql.ResultSet rsTable_tDBOutput_1 = dbMetaData_tDBOutput_1.getTables(null, null, null,
					...
					while (rsTable_tDBOutput_1.next()) {
						...
						if (table_tDBOutput_1.equals(("Dim_Collection"))
				...
				try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
					stmtCreate_tDBOutput_1.execute("CREATE TABLE \"" + tableName_tDBOutput_1
							+ "\"(\"ID_Collection\" INT4 ,\"Collection\" VARCHAR(255)  )");
				}
				String insert_tDBOutput_1 = "INSERT INTO \"" + tableName_tDBOutput_1
						+ "\" (\"ID_Collection\",\"Collection\") VALUES (?,?)";
...
// ###############################
				...
				if (execStat) {
					runStat.updateStatOnConnection(resourceMap, iterateId, 0, 0, "roow");
				}
				...
				final String decryptedPassword_tDBInput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:rgpRyo9F7ns0UXSXqXAw0faQhAkkGOQWqN7IzOO65z64CW0C");
				...
				try {
					...
					while (rs_tDBInput_1.next()) {
						...
						row2.Collection = StringUtils.replaceAllStrictly(row2.Collection, searchStr_tReplace_1_1,
								"" + "", false, false);
						String searchStr_tReplace_1_2 = " " + "";
						row2.Collection = StringUtils.replaceAllStrictly(row2.Collection, searchStr_tReplace_1_2,
								"" + "", false, false);
						roow.Collection = row2.Collection;
						...
						ope_tFilterRow_1.matches(
								(roow.Collection == null ? false : roow.Collection.compareTo("202x") != 0),
								"Collection.compareTo(\"202x\") != 0 failed");

						if (ope_tFilterRow_1.getMatchFlag()) {
							if (row4 == null) {
								row4 = new row4Struct();
							}
							row4.Collection = roow.Collection;
...
// Start of branch "row4"
						if (row4 != null) {
							...
							row3 = null;
							if (row4.Collection == null) {
								finder_tUniqRow_1.Collection = null;
							} else {
								finder_tUniqRow_1.Collection = row4.Collection.toLowerCase();
							}
							...
							if (!keystUniqRow_1.contains(finder_tUniqRow_1)) {
								...
								if (row4.Collection == null) {
									new_tUniqRow_1.Collection = null;
								} else {
									new_tUniqRow_1.Collection = row4.Collection.toLowerCase();
								}
								...
								row3.Collection = row4.Collection;
...
// # Output table : 'out1'
									out1_tmp.ID_Collection = Numeric.sequence("s1", 1, 1);
									out1_tmp.Collection = row3.Collection;
									out1 = out1_tmp;
// ###############################
...
// Start of branch "out1"
								if (out1 != null) {
									...
									if (out1.ID_Collection == null) {
										...
										pstmt_tDBOutput_1.setInt(1, out1.ID_Collection);
									}

									if (out1.Collection == null) {
										pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
									} else {
										pstmt_tDBOutput_1.setString(2, out1.Collection);
									}
				...
				if (execStat) {
					runStat.updateStat(resourceMap, iterateId, 2, 0, "roow");
				}
...
// ###############################      
	...
	public static void main(String[] args) {
		final Dim_Collection Dim_CollectionClass = new Dim_Collection();

		int exitCode = Dim_CollectionClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Dim_Collection.class.getClassLoader()
					.getResourceAsStream("restyle_up/dim_collection_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Dim_Collection.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : Dim_Collection");
		}
...
/************************************************************************************************
 * 97809 characters generated by Talend Open Studio for Data Integration on the
 * 12 avril 2025 à 8:26:46 PM CET
 ************************************************************************************************/